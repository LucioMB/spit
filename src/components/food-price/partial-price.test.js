...
jest.mock("../show/food.js", () => ({
    affettati: [{ price: 10 }, { price: 1231 }, { price: 648584846684 }]
}));

describe("PartialPrice", () => {
    it("should displays a partial price for change quantity in Counter", () => {
        const cmp = shallow(
            <PartialPrice currentPartialPrice={price * quantity} />
        );
    ...
    it("should display a number higher 0 for counter different to 0, but equal to 0 for counter 0, don't accept negative number", () => {
        const cmp = shallow(
            <PartialPrice currentPartialPrice={currentPartialPrice} />
        );
